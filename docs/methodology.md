...
### 26. 多模型编排不是多写手,而是分权协作 / Orchestrate models by authority, not headcount

多模型协作的价值不在"让更多 agent 同时改文件",而在把不同模型放进**不同权限与职责层级**,并让模型先自主提升自己:主 agent 保留上下文、代码主权、最终裁决和 Git 权限;可由仓库/manifest/docs/diff/命令发现的事实先自主求证;外部模型与子代理承担分析、规划、探索、审查和复核,按复杂度主动派发且不需要额外用户确认;写入型 worker 只在用户已明确要求实现/修改/落地/应用补丁、方案完全确定、文件范围互斥、边界清楚后才短时启用。这样既利用并行思考扩大证据覆盖,又避免多个模型同时拥有写权限造成上下文漂移、重复修改和 Git 污染。

源工程在 2026-06-30 连续两次把这条经验固化成规则:提交 `05c512d` 先区分"只读审查模式"与"已授权实施模式",并把子代理协作引入 auto-workflow;提交 `9001705` 再把它升级为独立的 `subagent-delegation.md`,形成 L0-L3 复杂度矩阵、只读子代理输出契约、复审子代理、实施子代理门和 Git 确认门。这条演化说明:多模型编排应先从真实协作事故中抽出权限边界,再把边界做成可触发、可审查、可降级的规则。

可复用的编排结构如下:

- **主控持权,外援增智**:主 agent 负责最终上下文、证据核验、代码整合、用户沟通和 Git 流程;外部 Codex / planner / architect / explorer / reviewer 只给分析、方案、风险和复核意见。
- **先自主求证,最小提问**:能从代码、manifest、docs、git diff、命令输出发现的事实不问用户;提问只用于外部不可证事实、安全边界、硬件/物理风险、worker 越权风险和 Git/远端历史变更。
- **主动只读并行,但管理并发槽位**:调研、评估、计划、复杂问题拆解、高风险审查默认积极委派只读子代理;成本不应成为减少只读复核的理由,也不需要用户为只读派发额外确认。主 agent 派发前清理完成代理,按批次派发并保留复审槽位,`wait` 后及时 `close`,防止占满并发上限。
- **复杂度分档**:L0 简单局部主 agent 自查即可;L1 至少一个只读 reviewer 查遗漏;L2/L3 涉跨核、ISR、`volatile`、共享内存、协议、安全、删除、行为保持重构或规则治理时,并行 explorer / architect / reviewer,再加复审子代理。
- **复审链闭合**:子代理结论不能直接采信;先由复审子代理检查证据充分性、矛盾和过度推断,再由主 agent 用 grep/读码/只读命令亲验关键点。
- **复审迭代到满意**:复审子代理给 FAIL、CRITICAL/HIGH 或影响规则一致性的 MEDIUM 时,主 agent 先自主修复可证问题,再重新派发只读复审;直到所有复审子代理对当前 change set 给 PASS,或剩余项属于 A3 用户确认/物理验证/平台限制并明确列出。
- **写入权后置**:实施 worker 只在调研充分、方案决策完整、文件范围互斥、接口/行为预期清楚且用户已明确要求实现/修改/落地/应用补丁后使用;每个 worker 必须限定目标文件、改动边界、验证方式和禁止事项。
- **Git 永不下放**:子代理不得 stage/commit/push;主 agent 完成实现和验证后只报告变更摘要、验证结果、建议提交说明和待暂存文件,等待用户确认当前 change set 后才进入 Git 写流程。
- **自主只读/实施模式分离**:用户要求查看、审查、调研、计划、解释时默认自主只读探索;只有明确要求修改、实现、落地、应用补丁或开始实施时才进入可写模式。
- **经验升级回路**:一次协作问题先沉淀为 lesson;反复出现或跨 target 通用时升级为规则;可机械检查时并入 auto-workflow,让"多模型协作纪律"从口头提醒变成常驻约束。

验收这套编排时看两类场景:行为保持重构必须触发只读对抗审查 + 复审 + 主 agent 亲验;跨核/ISR/协议任务必须触发 L2/L3 多路只读调研、复审子代理和 Git 确认门。若 `hf-auto-workflow`、`AGENTS.md`、`orchestration-contract.md` 之间出现"子代理可写 Git"或"只读任务自动改文件"这类缝隙,说明编排边界已经失效,应先修规则再实施。

→ 落地:包级唯一契约 `skills/hecateflow/references/orchestration-contract.md`,`hf-review`/`hf-refactor` 的子代理对抗审查要求、`hf-auto-workflow` 的编辑后复杂度分档、`hecateflow` 的自主只读/Git 红线。若目标工作区有独立规则层,建议从该 contract 派生 `subagent-delegation` 类规则,并在 manifest `interaction` / `git` 字段记录自主只读默认、实施 worker 门和 Git 确认门。

### 27. 文件分割:硬件底层 / 硬件顶层 / 软件实现分开封装 / Split complex features by layer

复杂功能不应继续塞进一个热点文件。源工程里已经形成了可复用形态:ToF 相关代码把 `tofBus`/vendor platform/API 与 `altitudeTof` facade、`altitudeHold` 控制逻辑分开;飞控与底盘代码也把 `pinMap`/参数头、驱动 owner、控制算法和应用循环放在不同目录。经验是:硬件底层只管 pin/bus/register/vendor adapter/init/config;硬件顶层提供 device facade、统一单位/状态/错误语义和 owner API;软件实现只放算法、控制和业务策略,依赖顶层接口而不直接摸寄存器或引脚。

行数是结构退化的早期信号。自写业务 `.c/.h` 超过 **650 行** 时,新增功能前必须做分文件评估;超过 **1000 行** 默认先拆分或封装管理。例外也要有边界:vendor / SDK / generated / 寄存器映射 / buffer table / 字库表等特殊长文件可保留;自写特殊长文件也可在用户明确确认后不拆,但必须记录为什么不拆、谁维护、后续增长边界。反过来,不足 650 行但会高频复用的 PID、滤波器、协议解析、数学工具或设备 facade,可以提前封成公共库或公共头,避免后续复制扩散。

→ 落地:`../references/embedded-c-style.md` 写三层封装与 650/1000 行规则;`hf-design-module` 在设计卡中列硬件底层/硬件顶层/软件实现和公共库候选;`hf-implement` 写前检查目标文件行数;`hf-auto-workflow`/`hf-review` 把 >650 未评估列 MEDIUM、>1000 未拆或无用户确认例外列 HIGH;`hf-refactor` 支持行为保持分文件拆分;模板把分层/行数门写进设计卡、实施计划和 PROJECT.md。

### 28. 通信先限权再提速:唯一 master / request-response / 有效帧 / 超时 / Communication is authority and freshness first

半双工或共享总线协议不要让多个节点周期性自发占线。源工程在 RS-485/LoRa 链路上把通信收敛成"唯一 master 发起请求,从机只在收到有效命令后应答",再配套 timeout、valid frame、链路年龄与失链降级;这样总线拥有者清楚,也能避免从机周期自发发送把半双工链路占满。通信越接近控制闭环,越要把"谁能发、何时发、旧帧何时失效"写成协议契约,而不是靠调用习惯。

通信中断风暴要隔离。源工程曾把 RS-485/LoRa 收发迁到低关键通信核:UART RX 中断只把字节放进缓冲,前台 `Poll` 限额排空解析,再用共享内存快照桥接控制核;RS-485 悬空/噪声造成的 RX 风暴只拖慢通信核,不再饿死 500us 控制 PIT。单核工程也适用同一原则:ISR 只收字节/入环形缓冲/置标志,前台有 byte/frame budget,协议解析和发送不在 ISR 热路径。

跨核、跨 ISR 或跨任务传命令时,用整块快照而不是零散字段。快照应有 `magic` 防上电垃圾、`seq`/单调计数防重复消费或漏消费、freshness gate/timeout 防旧帧继续驱动控制;失链时不要在高频热路径继续读取命令快照,先置零/软停/降级,避免临界区和通信时序耦合导致卡死或脏数据混帧。外部命令的微分/前馈也只能在新 `rxSeq` 边沿计算;首帧、超时或失链恢复时先重新锚定,旧帧产生的速度/前馈要衰减或清零。

→ 落地:`hf-design-module` 在通信模块设计卡中列 master、request-response、timeout、valid frame、rxSeq/freshness 与失链降级;`hf-embedded-safety` 把通信 ISR budget、快照 magic/seq、失链跳过命令快照列为安全门;`hf-review` 审查跨核/协议改动是否有只读复核;PROJECT 模板登记共享快照与链路新鲜度。

### 29. 多来源控制同一物理输出要有显式仲裁 / Multiple sources need an arbitration contract

单一 owner 不只意味着"只有一个模块写硬件",还意味着所有想控制同一物理输出的来源必须进入同一个仲裁器。源工程蜂鸣器把失控锁定、事件确认音、常规 pattern、静音提示统一到 owner 内,并写明优先级 `runaway > event > pattern > silent`;调用方只提交意图,不直接抢引脚。否则看似每个模块都只写一个小功能,实际运行时就是"谁最后写硬件谁赢",安全告警可能被普通提示音覆盖。

→ 落地:`hf-hw-mapping` 的驱动 owner 规则增加"多来源输出优先级";`hf-embedded-safety` 将执行器/蜂鸣/灯带/屏幕等共享物理输出的仲裁缺失列 HIGH;`hf-design-module` 要在设计卡里写 owner、来源列表、优先级和降级策略。

### 30. 参数持久化:默认值先行,只有可迁移版本才写回 / Persistent params must fail closed

运行时可调参数落 flash 时,不要只存 payload。源工程把三核参数 blob 固化为 `magic/version/payloadBytes/CRC32`:上电先加载编译期默认值,再读 flash 校验覆盖;`magic` 不符、`payloadBytes` 不符或 CRC 损坏都保留默认值且不自动覆盖 flash,避免把别核 blob、乱码或硬件故障当可写数据。只有 `magic` 对且 `version` 不符时,才走已登记窄迁移或在持久化开关允许时写回当前版本默认,让版本升级后自愈。

flash erase/write 是阻塞且有寿命的低频动作,只能在 init、PIT 启动前、`initReadyFlag` 守卫下或其它低风险阶段做,不进 ISR/控制热路径。改参数结构时要同步:结构字段、`*_FLASH_VERSION`、默认宏、迁移登记、PROJECT/参数持久化文档。TUNE/调参构建若关闭持久化,迁移也必须受 gate 约束。

→ 落地:`templates/config-header.h.tmpl` 写 `magic/version/payloadBytes/CRC` 与写回门;`hf-doc-discipline` 把参数持久化表纳入同步矩阵;`hf-embedded-safety` 拦截 flash 写入热路径;PROJECT 模板保留参数持久化记录。
...
- **工具名映射**:正文用 Claude 工具名,每个 SKILL.md 末尾"平台差异"段写 Codex 等价;共享 reference 可用相对路径读取。Codex 多代理工具可用时主动派只读子代理;若宿主策略限制无显式请求派代理,主会话降级执行并声明平台限制。
