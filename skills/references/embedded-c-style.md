...
- 多个小文件优于少数大文件:单文件典型 200–400 行;自写业务 `.c/.h` 超过 **650 行** 必须评估是否分文件;超过 **1000 行** 默认拆分或封装管理,特殊长文件经用户明确确认可不拆。
- **1000 行特殊长文件例外**:vendor / SDK / generated / 寄存器映射 / buffer table / 字库表等可保留;自写特殊长文件也可在用户明确确认后不拆,但必须记录"为何不拆、谁维护、后续增长边界"。
- **不足 650 行也可提前封装**:PID、滤波器、协议解析、数学工具、设备 facade、跨 target 高频复用能力,即使当前很小也可以拆成公共库或公共头,避免后期在热点文件里复制粘贴。
...
- **复杂功能三层分开封装**:硬件底层、硬件顶层、软件实现分离。硬件底层只管 pin/bus/register/vendor adapter/init/config;硬件顶层提供 device facade、统一单位/状态/错误语义、代码级 owner API;软件实现放算法、控制、业务策略,只依赖硬件顶层接口,不直接摸寄存器、引脚或 vendor 细节。
