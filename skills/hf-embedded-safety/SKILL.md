...
- 派子代理:Claude Code 用 `Task`(`subagent_type: "code-reviewer"`);Codex 在多代理工具可用且用户明确授权时用 `multi_agent_v1.spawn_agent`→`multi_agent_v1.wait_agent`→`multi_agent_v1.close_agent`,否则主会话逐项复核并声明未做子代理复核。
