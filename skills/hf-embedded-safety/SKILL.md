...
description: >
  通用嵌入式安全审查:ISR/volatile 共享数据、ISR 轻量性、通信中断风暴隔离与共享总线 request-response、
  跨核/跨 ISR 快照 magic/seq/freshness gate、数值钳位与除零/溢出/积分饱和、
  最内环失控锁定(必落所有模式必经输出关口)、独占外设白名单门控、驱动代码级 owner 与多来源输出仲裁、驱动配置 setter 惰性语义、
  ...
  Li005 / 电机失控 / 堵转 / PWM 限幅 / 中断里 printf / 屏幕乱码 / 总线抢占 / 半双工 / request-response /
  共享快照 / magic / seq / freshness / 通信中断风暴 / 参数持久化 / flash 写入 /
...
- **半双工/共享总线无唯一 master**:多个节点周期自发发送或从机未经请求应答 → 总线占满、碰撞、旧帧被当新命令。共享总线默认唯一 master + request-response + timeout + valid frame。
- **通信 ISR/轮询无预算**:UART RX 悬空或噪声造成中断风暴,若 ISR 做解析/发送或前台无限排空,会饿死控制 PIT。ISR 只收字节/入缓冲/置标志,前台解析必须有 byte/frame budget。
- **跨核/跨 ISR 命令无新鲜度门**:共享命令快照缺 `magic`/`seq`/freshness gate,或失链态仍在高频热路径读取旧命令 → 脏数据混帧、重复消费、甚至临界区与通信时序耦合卡死。
- **多来源输出无仲裁优先级**:蜂鸣器/灯带/执行器等同一物理输出被多个来源提交意图却没有 owner 内优先级(如 runaway > event > pattern > silent) → 安全告警可能被普通模式覆盖。
- **flash 写入进热路径**:参数持久化 erase/write 阻塞且有寿命,不得进 ISR/控制热路径;参数 blob 校验必须 fail closed,CRC/magic 错不自动覆盖。
...
4. 若改了通信/协议/共享内存,核对唯一 master、request-response、timeout、valid frame、ISR/轮询 budget、`magic`/`seq`/freshness gate 与失链降级。
5. 找出所有执行器输出点,确认钳位 + 除零保护 + 溢出边界;多来源控制同一输出时确认 owner 内有显式优先级仲裁。
6. 若改了参数持久化或 flash 写入,核对 blob 校验字段、写回门、CRC/magic 错误不覆盖、写入不在 ISR/控制热路径。
7. 跑下方 PASS/FAIL 清单,CRITICAL/HIGH 立即修(交回 `hf-implement`/`hf-auto-workflow` 落地)。
8. 关键并发/边界改动按 `../hecateflow/references/orchestration-contract.md` 分档;L1 自动派至少一个只读 reviewer,L2/L3 无论能否编译都必须多路只读对抗复核 + 复审链 + 主 agent 亲验。编译通过只是额外证据,不能替代复审。
...
- **半双工/共享总线**:优先唯一 master 发起,从机只在收到有效请求后应答;协议必须有 timeout、valid frame、链路年龄和失链降级。禁止多个节点周期自发发送把半双工链路占满。
- **通信中断风暴隔离**:ISR 只读字节、入缓冲、置标志;前台 `Poll`/解析必须有 byte/frame budget。若通信噪声会影响控制时序,优先迁到低关键核/低优先级任务,再用快照桥接控制核。
- **共享快照新鲜度**:跨核/跨 ISR 命令快照用整块复制,至少包含 `magic`/`seq` 或等价 freshness gate;失链/超时/`magic` 未就绪时不读旧命令,先置零、软停或降级。单按/边沿事件用单调计数交付,避免跨核 read-and-clear 竞态。
- **外部命令微分/前馈**:只在新 `rxSeq` 边沿计算;首帧、超时、失链恢复时重新锚定,旧帧前馈衰减或清零,避免 stale command 反复制造冲击。
- **参数持久化 fail closed**:flash blob 用 `magic/version/payloadBytes/CRC`。上电先 load defaults,再用校验通过的 blob 覆盖;只有 version 不符且迁移/持久化 gate 允许时写回。`magic` 不符、`payloadBytes` 不符、CRC 损坏默认不写 flash。
- **flash 写入时机**:erase/write 阻塞且有寿命,只在 init、PIT 启动前、`initReadyFlag` 守卫下或其它低风险阶段执行,不得进 ISR、控制 tick 或通信热路径。
...
- [ ] **多来源输出有仲裁**:同一物理输出若有多个来源(失控告警/事件音/常规 pattern/静音等),所有来源进入同一 owner 仲裁器,优先级显式且安全态最高。
- [ ] **驱动 setter 时序正确**:依赖硬件命令的配置(如屏方向 MADCTL)按驱动惰性语义放置——set-before-init 或 init 后补 `clear`/重绘;纯方向变更走 set-before-init。
- [ ] **通信协议限权**:半双工/共享总线有唯一 master、request-response、timeout、valid frame 与失链降级,无从机周期自发占线。
- [ ] **通信 ISR 有预算**:UART/总线 ISR 只收字节/入缓冲/置标志;前台解析有 byte/frame budget,通信风暴不会饿死控制 tick。
- [ ] **共享快照新鲜**:跨核/跨 ISR 快照含 `magic`/`seq` 或等价 freshness gate;失链/超时不读旧命令,边沿事件不用 read-and-clear 跨核交付。
- [ ] **外部命令导数只看新帧**:微分/前馈只在新 `rxSeq` 边沿计算,首帧/超时重新锚定,旧帧前馈衰减或清零。
- [ ] **参数持久化 fail closed**:blob 含 `magic/version/payloadBytes/CRC`;先 load defaults,CRC/magic/payloadBytes 错不自动覆盖 flash;flash 写入只在 init/低风险阶段。
...
- **半双工总线从机自发占线**:从机周期上报看似简单,实际会和 master 命令碰撞/占满链路。改唯一 master request-response 后,链路时序和失链判定才可控。
- **通信中断风暴拖死控制核**:RS-485 悬空噪声触发 RX 风暴,若通信在控制核热路径里解析,500us 控制 PIT 被饿死。迁到通信核/低优先级任务 + 前台 budget + 快照桥接可隔离。
- **失链态继续读命令快照**:链路不活时仍在高频 PIT 读旧命令/进临界区,可能和通信时序耦合卡死;应先看 freshness,失链置零/软停/降级。
- **参数 CRC 错自动覆盖 flash**:把损坏 blob 当"需要修复"直接写回,可能掩盖 flash 故障或误写别核数据。只有可迁移 version mismatch 才写回,CRC/magic 错 fail closed。
