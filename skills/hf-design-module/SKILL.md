...
在动手写代码前,把"要加的模块"想清楚:能不能复用现成的、接口长什么样、放在哪个功能子目录、会触及哪些容易漏的切点、涉不涉及极性/数量级/独占 IO 外设、要不要先在 PC 仿真。本 skill **只产出设计卡 + 计划文件,不写源码**(适合在 plan mode 跑);落地交 `hf-implement`。
...
设计模块 / 加功能 / 新模块 / 切点 / 要不要先仿真 / 极性 / IO 归属 / design module / plan feature。
...
5. **切点清单**(用 `templates/module-design.md.tmpl`),逐项列全:
   ...
   - ISR 路由 / 周期(若挂中断)。
   - **外设所有权门控**(若占独占 IO 外设 → 白名单 `#if` + 分核规划提示,见步 6)。
   - 共享数据 volatile(若跨 ISR/核)。
   - **极性 / 数量级**(若碰执行器/传感器/闭环/增益 → 见步 7)。
   - 文档同步(PROJECT.md 模块清单 + 边界)。
6. **IO 外设归属检查(点 13)**:模块若占用单实例 IO 外设——核对 manifest `targets[].ownedPeripherals`:本 target 是否 owner?门控是否白名单 `#if`(非黑名单,防新增模式默认抢占)?**主动提示该外设多核归属与分核规划**(其它核如何让出)。门控机制细节归 `hf-embedded-safety`,本步只在设计期标注切点。
