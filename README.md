...
v1.2 深化的工程纪律(均 MCU 无关):**自主性优先多模型编排**(先自主求证、主动派发只读子代理、复审链闭合、复审 FAIL 后反复修复再复审直到全 PASS 或只剩 A3 用户确认事项、最小提问只限 external unverifiable facts / safety boundaries / hardware/physical risk / write-worker scope escalation risk / Git/remote history changes、写入 worker 后置受限、Git 永不下放且必须用户确认)、**固化场景**(把赛规/安全约束写进 manifest 常驻贯穿)、**单工作区多工程管理**(关键词→target 路由 + 同名异义登记 + 共享库版本登记)、**分级分布式文档**(三层按需下钻省上下文)、**文件分割与三层封装**(硬件底层 / 硬件顶层 / 软件实现分开,650 行评估,1000 行需拆分或用户确认特殊长文件例外,<650 行但高频复用的能力可提前封成公共库/公共头)、**通信与共享快照安全**(半双工/共享总线唯一 master + request-response + timeout + valid frame,ISR 只收字节/入缓冲,前台限额解析,跨核快照用 magic/seq/freshness gate)、**参数持久化防误写**(magic/version/payloadBytes/CRC,先加载默认值,仅 version 迁移/覆盖写回,CRC/magic 错不自动覆盖,flash 写入只在 init/低风险阶段)、**自动进化学习**(代码改动→必同步文档矩阵)、**经验记忆不再犯**(本地 `.hecateflow/lessons/` 跨平台 record→recall→avoid→promote)、**事实来源二次确认**(用户/SDK/历史注释/既有代码都只是证据来源,"不可能出现"须复核后才采信)、**硬件映射头 + 参数头 + 极性单一真相源**(三组方向系数 + 开环辨识 + 禁藏 Kp 负号 + 闭环轴向对齐 + 数量级理智检查)、**硬件驱动代码级单一 owner**、**自动注入**、**clangd 配置管理**、**IO 外设多核归属 + 分核任务规划**、**相对路径优先**。详见 [`docs/methodology.md`](docs/methodology.md) 与 [`skills/hecateflow/references/orchestration-contract.md`](skills/hecateflow/references/orchestration-contract.md)。
