...
把 `skills/` 安装到 `~/.claude/skills`、`~/.codex/skills`、`~/.agents/skills` 与已初始化的 Qoder/QoderCN skill root(模板随 `hecateflow` 入口捆绑)。安装器会把 `~/.agents/skills` 登记到 Reasonix `[skills].paths`,并把 Qoder/QoderCN hook 写入对应 `settings.json`。幂等,可重复运行升级。
...
`install.ps1` / `install.sh` 默认会安装这个 hook。它不会直接修改代码,只在 Claude Code `Write`/`Edit`/`MultiEdit` 或 Qoder `create_file`/`search_replace` 等写文件工具后注入上下文,要求立即运行或说明跳过 `hf-auto-workflow`。
...
Codex 与 Reasonix 当前按无标准外部 hook 处理,`hf-auto-workflow` 靠 skill 正文约定"编辑后立即自审"。

## 跨平台说明 / Cross-platform

- skill 正文用 Claude Code 工具名书写;每个 SKILL.md 末尾"平台差异"段已内联 Codex 等价。Codex 多代理仅在工具可用且用户明确授权时使用 `multi_agent_v1.spawn_agent` 等,否则主会话降级执行。Reasonix 与 Qoder 按当前会话实际暴露的工具执行,无等价工具时回退为主会话自审。完整映射见 [`skills/hecateflow/references/`](skills/hecateflow/references/)。
